...
function runProgram() {
  ...
  let chaserCurrentSpeed = 3.2
  let originalSpeed = chaserCurrentSpeed
...
/*
  while (!mode) {
    ...
    else if (bOF == "p" || bOF == "player" || bOF == "friend" || bOF == "f" || bOF == "amigo" || bOF == "") { mode = "player" }
  ...
  function handleKeyDown(event) {
    ...
    if (mode == "bot") {
      if (event.which === KEY.R) {
        if (empReady) {
var audio = new Audio('/walker/sounds/teleport-14639.mp3')
audio.play()
         timesPopped = timesPopped + 1
          ...
          function triggerEMP() {
            ...
            if (empDuration == 5) {
              // slow the chaser
              empVariation = 1
              chaserCurrentSpeed = originalSpeed/2
            }

            if (empDuration == 10) {
              empVariation = 0
              chaserCurrentSpeed = originalSpeed
  ...
  function handleKeyUp(event) {
    ...
    if (event.which === KEY.DOWN) {
      ...
      if (isxPossible == true) { walker.ySpeed = 0 }
    }
  ...
  function setup() {
    var scanning = true
    $("#subboard").css("display", "none")
    var plrSetupL = $("#playerSetup").css("left")

    $("#botSetup").css("left",plrSetupL)

    walker.x = 1070 / 2
    walker.y = 450

   var bb = setInterval(scan,10)
   ...
   function scan() {
    ...
    if (570 < parseFloat(walkerz.css("top")) + parseFloat(walkerz.css("height")) && 630 > parseFloat(walkerz.css("top")) && 370 < parseFloat(walkerz.css("left")) + parseFloat(walkerz.css("width")) && 730 > parseFloat(walkerz.css("left"))) {
 mode = "player"
 $("#subboard").css("display", "block")
 ...
 setEverything()
    ...
    if (250 < parseFloat(walkerz.css("top")) + parseFloat(walkerz.css("height")) && 310 > parseFloat(walkerz.css("top")) && 410 < parseFloat(walkerz.css("left")) + parseFloat(walkerz.css("width")) && 770 > parseFloat(walkerz.css("left"))) {
  ...
  function handleTimer() {
    ...
    $("#timer").text(timer)
    if (mode == "bot") {
      $("#timer").text(timer + " DIFF: " + Math.round(botDifficulty * 10))
    }
    if (timer == 0) {
      timer = ogtimer
      $("#timer").text(timer)
...
setup()
  ...
  function endGame() {
    ...
    if (mode == "bot") {
      ...
      if (health == 2) { $("#hearts").text("❤️  ❤️") }
      else if (health == 1) { $("#hearts").text("❤️") }
      else if (health == 0) {
        $("#hearts").text("")
        score = score - timesPopped * 5

        let judgement
        let usedEmps
        if (score <= 0) { judgement = "Did you lose on purpose?" }
        else if (score <= 10) { judgement = "Not horrible, but you could do better." }
        else if (score >= 11 && score <= 49) { judgement = "Pretty good!!" }
        ...
        else if (score >= 1000) { judgement = "Ur crazy" }

if (timesPopped == 0) {
  usedEmps = "Wow, you didn't even use any!"
}
else { usedEmps = "EMPS aren't cheap!"}

        alert("Your final score was: " + score + "\n \n You activated EMP " + timesPopped + " times, resulting in a score alteration of " + -timesPopped * 5 + ". "+usedEmps+ "\n \n " + judgement)
        location.reload()
